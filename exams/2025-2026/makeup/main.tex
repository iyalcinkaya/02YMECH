...
\title{{\large Department of Physics, FNSPE, CTU in Prague} \\
                  02YMECH - Mechanics \\ 
          {\Large \textbf{Make-up / Midterm 2}}\vspace{-1cm}}
\date{9.1.2026 / 14:00 - 15:30}
\author{}
...
\textbf{Q1:} Consider a system of two point particles, each having identical masses $m$, moving on a $xy$ plane with accelerations, $\vec a_{1}~=~(0,a)$ and $\vec a_{2}~=~(a/2,0)$ where $a$, is a positive constant. The particles are \underline{initially at rest} in positions, $\vec r_{1}(0)~=~(1,0)$ and $\vec r_{2}(0)~=~(0,2)$.

\vspace*{0.1cm}
~(a) Find the total torque about the origin and the total force. --- \textit{0.4 points}
...
\vspace*{0.25cm}
\textbf{Q2:} The equation of motion of a damped oscillator is given as $x(t)=e^{-\beta t}x_0\cos{\left(\omega_1 t\right)}$. How many oscillations $N$ does it approximately take for the amplitude of the oscillation to reduce to $1/e$ if the quality factor $Q$ of the damped oscillator is $90$? Assume that the oscillator is weakly damped, i.e., $\omega_1 \approx \omega_0$.
(Givens: $Q = \omega_0/2\beta$, $T_0=2\pi/\omega_0$, $\pi\approx 3$) --- \textit{1 point}

\vspace*{0.25cm}
\textbf{Q3:} A particle of mass $\mu$ moves under a central force $f(r)$ with angular momentum $l$. Its orbit is given by $r(\theta) = \frac{1}{a \theta}$ where $a > 0$. The orbit of a particle under a central force satisfies
\[
